...
if __name__ == '__main__':
    ...
    lightPos2 = [1.0, -1.0, -1.0, 0.0]

    glEnable(GL_LIGHT1)
    glLightfv(GL_LIGHT1, GL_AMBIENT, [ambient, ambient, ambient, 1])
    glLightfv(GL_LIGHT1, GL_DIFFUSE, [4, 4, 4, 1])
    ...
    while True:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                ...
                quit()
        # clear all
        lightPos = [0.0, -0.2, -5.0, 1.0]
        glLightfv(GL_LIGHT0, GL_POSITION, lightPos)
        lightPos2 = [1.3, -0.2, -5.0, 1.0]
        ...
        # a2 = list(map(lambda x: [x[0], x[1], x[2]-0.1], a2))
        draw_figure(a1, a2)
        glPopMatrix()
        # # ellipsis
        glPushMatrix()
        ...
        if t2[0][2] > 2:
            deep = True
        if deep:
            t2 = list(map(lambda x: [x[0], x[1], x[2] - 0.3], t2))
        else:
            t2 = list(map(lambda x: [x[0], x[1], x[2] + 0.3], t2))
